...
contract Armors_token is setOperator, ERC721{

    using SafeMath for uint256;
    using SafeMath16 for uint16;

    // modifier onlychaContract{
    //     require(msg.sender == addr("chaToken") || msg.sender == addr("chaEx"), "You are not npc Contract");
    //     _;
    // }
...
//////////////initial equipment///////////////
    ...
    function _iniEquAbility(uint _id) private pure returns(uint8 rarity,uint8 star, int16 atk, int16 def, int16 hp){
        ...
        if(random0 < level){
            rarity = (level / 10) + 2;
            bytes1 random1 = random[13];
            bytes1 random2 = random[14];
            bytes1 random3 = random[18];
            bytes1 random4 = random[21];
            star = 0;    
            atk = (int16(random1) % 5 + 10 ) * (rarity - 1) + 10;
            def = (int16(random2) % 15 + 30) * (rarity - 1) ;
            hp =  ((int16(random3)+int16(random4)) % 20 + 100) * (rarity - 1) ;
        }
        else{
            rarity = 1;
            star = 0;
...
////////////////game function/////////////////
    ...
    function set_ability(uint256 tokenId,uint16 _id,uint8 amount) internal{
        ...
        (,,,,uint8 _rarity,uint8 _equ_type) = mixInterface(addr("mix")).inquireEquInfo(2,_id);
        (int16 atk_max,int16 atk_min,int16 def_max,int16 def_min,int16 hp_max,int16 hp_min) = mixInterface(addr("mix")).inquireEquAbility(2,_id);
        ...
        if(atk_max == 0 && atk_min == 0 ){
            ...
            equiptAbility[tokenId].atk = int16(random[12]) % (atk_max-atk_min+1) + atk_min;
            if(equiptAbility[tokenId].atk > 0){
        ...
        if(def_max == 0 && def_min == 0 ){
            ...
            equiptAbility[tokenId].def = int16(random[14]) % (def_max-def_min+1) + def_min;
            if(equiptAbility[tokenId].def > 0){
        ...
        if(hp_max == 0 && hp_min == 0 ){
            ...
            equiptAbility[tokenId].hp = int16(random[16]) % (hp_max-hp_min+1) + hp_min;
            if(equiptAbility[tokenId].hp > 0){
