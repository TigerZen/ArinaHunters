...
interface mixInterface{
    function inquireEquInfo(uint8 _type,uint _id) external view returns
    (string equipt_name,uint8[3] boxIndex,uint8[3] materialIndex,uint[3] amount,uint8 rarity,uint8 equ_type);
                                                      
    function inquireEquAbility(uint8 _type,uint _id) external view returns
    (int16 atk_max,int16 atk_min,int16 def_max,int16 def_min,int16 hp_max,int16 hp_min);
}

contract Weapons_token is setOperator, ERC721{
    ...
    // address public chaExContract;
    //address public synthesisContract;
   
    // address public mix_address ;
...
//////////////initial equipment///////////////
    ...
    function _iniEquInfo(uint _id) private pure returns(string equName, uint8 equiptType){
        ...
        equiptType = uint8(random[7]).mod(3).add(1);
        if(equiptType == 1){
            equName = "Sword";
        }
    ...
    function _iniEquAbility(uint _id) private pure returns(uint8 rarity,uint8 star, int16 atk, int16 def, int16 hp){
        ...
        if(random0 < level){
            rarity = (level / 10) + 2;
            bytes1 random1 = random[12];
            bytes1 random2 = random[15];
            bytes1 random3 = random[17];
            bytes1 random4 = random[20];
            star = 0;
            atk = (int16(random1) % 30 + 20 ) * (rarity - 1) + 10;
            def = (int16(random2) % 15) * (rarity - 1) ;
            hp =  ((int16(random3)+int16(random4)) % 100) * (rarity - 1) ;
        }
        else{
            rarity = 1;
            star = 0;
...
////////////////game function/////////////////
    ...
    function set_ability(uint256 tokenId,uint16 _id,uint8 amount) internal{
        ...
        (,,,,uint8 _rarity,uint8 _equ_type) = mixInterface(addr("mix")).inquireEquInfo(1,_id);
        (int16 atk_max,int16 atk_min,int16 def_max,int16 def_min,int16 hp_max,int16 hp_min) = mixInterface(addr("mix")).inquireEquAbility(1,_id);
        ...
        if(atk_max == 0 && atk_min == 0){
            ...
            equiptAbility[tokenId].atk = int16(random[6]) % (atk_max-atk_min+1) + atk_min;
        ...
        if(def_max == 0 && def_min == 0 ){
            ...
            equiptAbility[tokenId].def = int16(random[8]) % (def_max-def_min+1) + def_min;
        ...
        if(hp_max == 0 && hp_min == 0 ){
            ...
            equiptAbility[tokenId].hp = int16(random[10]) % (hp_max-hp_min+1) + hp_min;
            if(equiptAbility[tokenId].hp > 0){
